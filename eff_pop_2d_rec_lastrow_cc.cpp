...
long double gridK[n_demesh][n_demesw] = {{0}};
	...
	while(calcLastRow(deme) < n_demesh-5){
		...
		for(int ii = 0; ii < int(n_demesh); ii++){
			...
			for(int jj = 0; jj < int(n_demesw); jj++){
				...
				if(deme[ii][jj][0] + deme[ii][jj][1] == gridK[0][jj]){
					full+=1;
				}	
		...
		for( int j=0; j<n_demesw; j++){
			...
			long double f1 = deme[0][j][0]/int(gridK[0][j]);
			cout <<deme[0][j][0]/int(gridK[0][j])<<endl;
			long double f2 = deme[0][j][1]/int(gridK[0][j]);
			f1 = (1 - M1*(3/4))*f1;
			f2 = (1 - M2*(3/4))*f2; 
			...
			for(int ne = 0; ne <3; ne++){
				...
				f1+= (M1/4)*deme_aux[neighbs[ne][0]][neighbs[ne][1]][0]/int(gridK[0][j]);
				f2+= (M2/4)*deme_aux[neighbs[ne][0]][neighbs[ne][1]][1]/int(gridK[0][j]);
			...
			new_prob[1] = f1;
			new_prob[2] = f2;
			gsl_ran_multinomial(r,n_spec+1,gridK[0][j],new_prob,new_cnt);
		...
		for(int i = d_start; i < d_end -1; i++){
			for(int j = 0; j < n_demesw; j++){
				...
				long double f1 = deme[i][j][0]/int(gridK[i][j]);
				long double f2 = deme[i][j][1]/int(gridK[i][j]);
				f1 = (1 - M1)*f1;
				f2 = (1 - M2)*f2; 
				for(int ne = 0; ne <4; ne++){

					f1+= (M1/4)*deme_aux[neighbs[ne][0]][neighbs[ne][1]][0]/int(gridK[i][j]);
					f2+= (M2/4)*deme_aux[neighbs[ne][0]][neighbs[ne][1]][1]/int(gridK[i][j]);
				...
				new_prob[1] = f1;
				new_prob[2] = f2;
				gsl_ran_multinomial(r,n_spec+1,gridK[i][j],new_prob,new_cnt);
