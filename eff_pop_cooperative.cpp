...
const int NDeme = 300; // Simulation box size
const unsigned int NSp = 2; // Number of species
unsigned int NGen = 1000; // Run time in generations 
...
int main(int argc, char * argv[])
{
    ...
    // Initialize profile
    //-----------------------------------------------------------------------------------//
    float fill = 0.5; // Initial filling fraction
    ...
    for (unsigned int t = 0; t < NGen; t++)
    {
        ...
        for (int i = 1; i < NDeme - 1; i++)
        {
            ...
            sp_frac[i][0] = (1 - M)*sp_frac[i][0] + (M/2.0)*holder_p0 + (M/2.0)*sp_frac[i + 1][0]; // Migrate
            holder_p0 = aux; // Assign previous value to holder
            aux = sp_frac[i][1];
            sp_frac[i][1] = (1 - M)*sp_frac[i][1] + (M/2.0)*holder_p1 + (M/2.0)*sp_frac[i + 1][1];
            holder_p1 = aux;
            //-----------------------------------------------------------------------------------//
        ...
        //-----------------------------------------------------------------------------------//
        sp_frac[NDeme - 1][0] = (1 - M/2.0)*sp_frac[NDeme - 1][0] + (M/2.0)*holder_p0;
        sp_frac[NDeme - 1][1] = (1 - M/2.0)*sp_frac[NDeme - 1][1] + (M/2.0)*holder_p1;
        ...
        if (t%tHetInterval == 0)
        {
            ...
            cout << instDemes + shiftDemes<<endl;
        }
